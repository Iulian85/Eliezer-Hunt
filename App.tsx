...
function App() {
    ...
    const checkDeviceSecurity = useCallback(() => {
        // Verificare pentru jailbreak/root
        const isJailbroken = () => {
        ...
        // Dacă sunt detectate probleme de securitate, se poate bloca accesul
        if (isJailbroken() || isDebuggerAttached()) {
            console.warn('Security issue detected: possible jailbreak/root or debugger attached');
            // Aici poți implementa logica pentru a bloca accesul sau avertiza utilizatorul
        }
    }, []);

    // Verificare securitate la încărcare
    useEffect(() => {
        checkDeviceSecurity();
    }, [checkDeviceSecurity]);
        ...
        <div className="h-screen w-screen bg-slate-950 text-white flex flex-col relative overflow-hidden">
            <div className="flex-1 relative overflow-hidden">
                ...
                {activeTab === Tab.ADS && (
                    <AdsView 
                        ...
                        onSubmitApplication={async (coords: Coordinate, count: number, mult: number, price: number, data: SponsorData) => {
                            const camp: Campaign = {
                                id: `camp-${Date.now()}`,
            ...
            <div className="fixed bottom-0 left-0 right-0 z-[9999] p-4 mb-2 pointer-events-none">
                <div className="pointer-events-auto">
                    <Navigation currentTab={activeTab} onTabChange={setActiveTab} userWalletAddress={userWalletAddress} isAdmin={isAdmin} />
                </div>
            </div>
        </div>
    );
