...
export const saveCollectionToFirebase = async (tgId: number, spawnId: string, value: number, category?: HotspotCategory, tonReward: number = 0) => {
    ...
    try {
        ...
        // Update user balance (appears instantly in wallet via onSnapshot)
        await updateDoc(userRef, updateData);

        // Create log in claims with status VERIFIED directly
...
export const getAdminWallet = async () => {
    try {
        const getAdminWalletFunc = httpsCallable(functions, 'getAdminWallet');
        const result: any = await getAdminWalletFunc();
        return result.data;
    } catch (e) {
        console.error('Error getting admin wallet:', e);
        return null;
    }
};
